...
class Processor {
    ...
    decodeInstruction(instructionByte) {
        ...
        if (this.debugInstructions) {
            console.log("FOUND INSTRUCTION:", instructionString, Log.getInstructionFromByte(instructionByte));
        }
    ...
    *_moveImmediateToRegister() {
        ...
        yield "MOV3" + ` - Register ${register._debugName()} has value = ${Log.getStringFromByte(register.getValue())}`;
    }
    ...
    *_moveRegisterToRegister() {
        ...
        yield "MOV3" + ` - Set ${registerB._debugName()} to value ${Log.getStringFrom16Bits(registerB.getValue())}`;
    }
    ...
    *_addRegisterToRegister() {
        ...
        yield "ADD3" + ` - Accumulator has now value = ${Log.getStringFrom16Bits(this.accumulator.getValue())} / Carry flag is ${carry}`;
    }
    ...
    *_compareRegisterToRegister() {
        const registerA = this.getRegister(this.fetchNextByte());
        const registerB = this.getRegister(this.fetchNextByte());
        yield "CMP1" + ` - Comparing Register ${registerA._debugName()} to Register ${registerB._debugName()}`;
    ...
    *_pushRegister() {
        ...
        const registerValue = register.getValue();
        this.push16(registerValue);
        yield `Pushed value from Register ${register._debugName()} - ${Log.getStringFrom16Bits(registerValue)}`;
    }
    ...
    *_popToRegister() {
        ...
        const register = this.getRegister(this.fetchNextByte());
        register.setValue(value);
        yield `POP3 - Register ${register._debugName()} has now value ${Log.getStringFrom16Bits(register.getValue())}`;
    }
}
...
loadToMemory(sampleProcessor.memory);
